...
export class AuthService {
  private static currentUser: ARCAUser | null = null;
  private static authListeners: ((user: ARCAUser | null) => void)[] = [];

  /**
   * Initialize the auth service and check for existing session
   */
  static async initialize(): Promise<ARCAUser | null> {
    if (!supabase) {
  ...
  static async register(data: RegisterData): Promise<AuthResult> {
    ...
    try {
      ...
      // Create user profile
      const userProfile: Partial<ARCAUser> = {
        id: authData.user.id,
        email: data.email,
   ...
   */
  static getCurrentUser(): ARCAUser | null {
   ...
   */
  static hasMinimumRole(minRole: UserRole): boolean {
    const user = this.getCurrentUser();
   ...
   * Update user profile
   */
  static async updateProfile(updates: Partial<UserProfile>): Promise<AuthResult> {
    const currentUser = this.getCurrentUser();
  ...
  static async requestRoleChange(
    ...
    try {
      ...
      const updatedUser = { 
        ...currentUser, 
        roleRequestStatus: 'pending' as const,
   ...
   */
  static addAuthListener(callback: (user: ARCAUser | null) => void): () => void {
  ...
  private static setCurrentUser(user: ARCAUser | null): void {
    this.currentUser = user;
    this.authListeners.forEach(listener => listener(user));
  }

  private static async fetchUserProfile(userId: string): Promise<ARCAUser | null> {
  ...
  private static mapDatabaseUserToARCAUser(dbUser: any): ARCAUser {
    return {
      id: dbUser.id,
      email: dbUser.email,
      role: dbUser.role,
  ...
  private static async createDefaultPermissions(userId: string, role: UserRole): Promise<void> {
    if (!supabase) return;
